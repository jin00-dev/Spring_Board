...
//      return mav;		
	...
	public void fileThumdDownloadGET(@RequestParam("fileName") String fileName,
			HttpServletRequest request,
			HttpServletResponse response) throws Exception{
		logger.debug(" fileThumdDownloadGET() 호출 ");
		...
		// 업로드 했던 (다운로드 할) 파일의 확장자 확인
		// 예를 들어 "itwill.jsp" 일때 .의 인덱스를 찾아라 
		int lastIdx = fileName.lastIndexOf(".");
		// 원본 파일에 대한 접근 후 확장자를 뺀 나머지 이름 저장 => 썸네일 이름으로 만들어 질 것 
		String thumbName = fileName.substring(0, lastIdx);

		// 파일명이 한글일때 인코딩문제 해결
//		fileName = URLEncoder.encode(thumbName,"UTF-8");
		
		File thumbNail = new File(request.getRealPath("\\upload")+"\\"+"thumbNail\\"+thumbName+".png");
		...
		//파일이 있는지 비교 
		if(file.exists()) {
//			thumbNail.getParentFile().mkdirs(); => 썸네일 폴더를 만들거에요
			// 썸네일 생성 
//			Thumbnails.of(file).size(50, 50).outputFormat("png").toFile(thumbNail); // 해당 파일에 대한 썸네일 만들 것
		}	// 썸네일 바로 출력 (파일 생성 X)
			Thumbnails.of(file).size(50, 50).outputFormat("png").toOutputStream(out);// 출력될 수 있게 함 
